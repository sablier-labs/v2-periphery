...
contract Withdraw_Unit_Test is Unit_Test {
    function test_RevertWhen_NotDelegateCalled() external {
        vm.expectRevert(Errors.CallNotDelegateCall.selector);
        target.withdraw({ lockup: linear, streamId: 0, to: users.recipient.addr, amount: 0 });
    }
    ...
    function test_Withdraw_Dynamic() external whenDelegateCalled {
        uint256 streamId = createWithMilestones();
        test_Withdraw(dynamic, streamId);
    }

    function test_Withdraw_Linear() external whenDelegateCalled {
        uint256 streamId = createWithRange();
        test_Withdraw(linear, streamId);
    }
