...
export default async function () {
  ...
  console.table({
    contract: "SablierV2Batch & SablierV2MerkleStreamerFactory",
    chainId: chainId,
    network: networkName,
  ...
  const artifactBatch = await deployer.loadArtifact("SablierV2Batch");
  const artifactMerkleFactory = await deployer.loadArtifact("SablierV2MerkleStreamerFactory");

  const batch = await deployer.deploy(artifactBatch, []);
  const batchAddress = batch.target.toString();
  console.log("SablierV2Batch deployed to:", batchAddress);
  ...
  console.log("SablierV2MerkleStreamerFactory deployed to:", merkleFactoryAddress);
